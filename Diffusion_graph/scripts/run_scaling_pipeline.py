...
def check_backbone_checkpoint(results_dir: Path) -> bool:
    ...
    checkpoint_path = results_dir / 'step1_backbone' / 'checkpoints' / 'graph_backbone_best.pt'
...
def main():
    ...
    # Log model config
    model_config = get_model_config(args.model_size, config, model_type='graph')
    training_config = get_training_config(args.model_size, config, model_type='graph')

    # Load graph tokenizer to get vocab_size for param estimation
    tokenizer_path = Path(config['paths']['results_dir']) / 'graph_tokenizer.json'
    vocab_size = 100  # Default estimate
    if tokenizer_path.exists():
        with open(tokenizer_path, 'r') as f:
            tokenizer_data = json.load(f)
            # Graph tokenizer has 'atom_vocab' dict
            if 'atom_vocab' in tokenizer_data:
                vocab_size = len(tokenizer_data['atom_vocab'])
            elif isinstance(tokenizer_data, dict):
                vocab_size = len(tokenizer_data)
    else:
        print(f"Warning: Graph tokenizer not found at {tokenizer_path}, using default vocab_size=100")

    num_params = estimate_params(model_config, vocab_size, model_type='graph')
